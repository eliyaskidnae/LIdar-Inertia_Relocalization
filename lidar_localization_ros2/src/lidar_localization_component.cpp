...
CallbackReturn PCLLocalization::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  if (set_initial_pose_)
  {
    ...
    // infoing the initial pose
    RCLCPP_INFO(get_logger(), "Initial pose received");
    RCLCPP_INFO(get_logger(), "Initial pose: [%f, %f, %f, %f, %f, %f, %f]",
                initial_pose_x_, initial_pose_y_, initial_pose_z_,
                initial_pose_qx_, initial_pose_qy_, initial_pose_qz_, initial_pose_qw_);
...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  double position_scale, orientation_scale;

  if (fitness_score < 0.5 * score_threshold_)
  {
    position_scale = 0.1;
    orientation_scale = 0.1;
  }
  else if (fitness_score < 0.8 * score_threshold_)
  {
    position_scale = 0.5;
  ...
  for (int i = 0; i < 6; ++i)
  {
    if (i < 3)
    {
      corrent_pose_with_cov_stamped_ptr_->pose.covariance[i * 6 + i] =
